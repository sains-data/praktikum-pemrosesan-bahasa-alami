...
class Voc:
    def __init__(self):
        self.trimmed = False   # Apakah kosakata sudah difilter frekuensi
        self.word2index = {}
        self.word2count = {}
        self.index2word = {PAD_token: "PAD", SOS_token: "SOS", EOS_token: "EOS"}
        self.num_words = 3  # Ukuran kosakata (awal: PAD, SOS, EOS)
    ...
    def trim(self, min_count):
        ...
        print('Kata dipertahankan {} / {} = {:.4f}'.format(
            len(keep_words), len(self.word2index), len(keep_words) / len(self.word2index)
        ))

        # Bangun ulang kosakata setelah filter
        self.word2index = {}
        self.word2count = {}
        self.index2word = {PAD_token: "PAD", SOS_token: "SOS", EOS_token: "EOS"}
...
def readVocs(corpus_path):
    """Memuat korpus dan mengembalikan objek kosakata serta pasangan kalimat."""
...
def filterPair(p):
    """Membuang pasangan yang melebihi panjang maksimum."""
    return len(p[0].split(' ')) < Config.MAX_LENGTH and len(p[1].split(' ')) < Config.MAX_LENGTH


def filterPairs(pairs):
    """Memfilter daftar pasangan kalimat."""
...
def loadPrepareData(data_path):
    print('Langkah 1: normalisasi teks dan baca korpus')
    voc, pairs = readVocs(data_path)
    print('Langkah 2: filter kalimat terlalu panjang ... ', end='')
    pairs = filterPairs(pairs)
    print('total {} pasangan'.format(len(pairs)))

    print("Membangun kosakata...")
    for pair in pairs:
        voc.addSentence(pair[0])
        voc.addSentence(pair[1])
    print("Jumlah token dalam kosakata:", voc.num_words)
    return voc, pairs
