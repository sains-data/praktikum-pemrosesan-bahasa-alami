...
class Seq2SeqModel(nn.Module):
    """Chatbot BERT: encoder BERT + kepala prediksi bahasa (gaya UniLM)."""
    ...
    def compute_loss(self, predictions, labels, target_mask):
        """Loss hanya pada segmen jawaban; PAD dan segmen pertanyaan di-mask."""
    ...
    def forward(self, input_tensor, token_type_id, position_enc=None, labels=None, device="cpu"):
        """Forward pass; jika labels diberikan, mengembalikan (predictions, loss)."""
        input_shape = input_tensor.size()

        seq_len = input_shape[1]
        # Mask perhatian: segmen A bidirectional, segmen B kausal (segitiga bawah)
        ...
        if labels is not None:
            # Loss: prediksi tidak termasuk token [SEP] terakhir
    ...
    def generate(self, text, out_max_length=50, beam_size=1, device="cpu"):
        """Menghasilkan jawaban untuk satu pertanyaan (beam search opsional)."""
    ...
    def beam_search(self, token_ids, token_type_ids, word2ix, beam_size=1, device="cpu"):
        """Decoding beam search token demi token."""
        sep_id = word2ix["[SEP]"]
        output_ids = [[]]
