...
def topk_filtering(logits, topk=10, topp=0.0, filter_value=-float('Inf')):
    ...
    if topk > 0:
        # Ambil top-k token dengan probabilitas tertinggi
...
if __name__ == '__main__':
    ...
    BertConfig.from_pretrained('./bert_pretrain')
    model = ClipCaptionModel()
    if torch.cuda.is_available():
        model.to(device)

    model.load_state_dict(torch.load('./output/base_model_epoch1_step0.bin', map_location=device))
    model.eval()
    model.half()
