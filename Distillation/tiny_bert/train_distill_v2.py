...
def evaluate(epoch):
    ...
    for input_ids, input_mask, segment_ids, label_ids, scores in tqdm(eval_dataloader, desc='Evaluation'):
        step += 1
...
def compute_loss(logits, labels_ids, scores, layer_13_output, layer_3_output):
    ...
    loss1 = F.cross_entropy(logits.view(-1, 2), labels_ids.view(-1))
    loss2 = kd_mse_loss(logits, scores)
    ...
    # print(loss1)
    # print(loss2)
    # print(layer1_mse_loss)
    # print(layer2_mse_loss)
    # print(layer3_mse_loss)
    loss = loss1 + loss2 + layer1_mse_loss + layer2_mse_loss + layer3_mse_loss
...
if __name__ == '__main__':
    ...
    teacher_model = Model()
    teacher_model.load_state_dict(torch.load('./save_model/epoch3_ckpt.bin', map_location='cpu'))
    ...
    if args.do_train:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                print('<distilasi> epoch:{}, step:{}, loss:{:10f}, waktu:{:10f}'.format(epoch, step, loss, time.time()-start_time))
                loss.backward()
