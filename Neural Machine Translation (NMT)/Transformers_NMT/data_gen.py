...
# Muat data.pkl dan catat waktu pemuatan
start = time.time()
...
def pad_collate(batch):
    """Padding batch; kembalikan (src_pad, tgt_pad, panjang_src_asli)."""
    max_input_len = float('-inf')
    max_target_len = float('-inf')

    for elem in batch:
        src, tgt = elem
        # Panjang maksimum dalam batch ini
    ...
    for i, elem in enumerate(batch):
        ...
        # (input padding, target padding, panjang input asli)
...
class TranslateDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        src_text = sample['in']   # belum di-pad; target sudah ada SOS/EOS
        tgt_text = sample['out']
